...
//==============================>
  ...
  setCountryBasedOnUserProfile(BuildContext context) {
    selectedCountry = Provider.of<ProfileService>(context, listen: false)
            ...
            .country
            ?.country ??
        asProvider.getString('Select Country');
...
//==============================>
  setStateBasedOnUserProfile(BuildContext context) {
    selectedState = Provider.of<ProfileService>(context, listen: false)
            ...
            .city
            ?.serviceCity ??
        asProvider.getString('Select State');
    ...
    if (countryDropdownList.isEmpty) {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        for (int i = 0; i < data.countries.length; i++) {
          if (!countryDropdownList.contains(data.countries[i])) {
        ...
        //error fetching data
        countryDropdownList.add(asProvider.getString('Select Country'));
        countryDropdownIndexList.add('0');
        selectedCountry = asProvider.getString('Select Country');
        selectedCountryId = '0';
        fetchStates(selectedCountryId, context);
        notifyListeners();
      }
  ...
  fetchStates(countryId, BuildContext context) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      for (int i = 0; i < data.serviceCities.length; i++) {
        if (!statesDropdownList.contains(data.serviceCities[i])) {
      ...
      fetchArea(countryId, selectedStateId, context);
      //error fetching data
      statesDropdownList.add(asProvider.getString('Select State'));
      statesDropdownIndexList.add('0');
      selectedState = asProvider.getString('Select State');
      selectedStateId = '0';
      notifyListeners();
    }
  }
  ...
  fetchArea(countryId, stateId, BuildContext context) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      for (int i = 0; i < data.serviceAreas.length; i++) {
        if (!areaDropdownList.contains(data.serviceAreas[i])) {
      ...
      areaDropdownList.add(asProvider.getString('Select Area'));
      areaDropdownIndexList.add('0');
      selectedArea = asProvider.getString('Select Area');
      selectedAreaId = '0';
      notifyListeners();
    }
  }
