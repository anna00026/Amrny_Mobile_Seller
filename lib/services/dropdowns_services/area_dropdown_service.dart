...
//==============================>
  ...
  Future<bool> fetchArea(BuildContext context, {bool isrefresh = false}) async {
    ...
    var response = await http.get(Uri.parse(
        '$baseApi/country/service-city/service-area/$selectedCountryId/$selectedStateId?page=$currentPage&order=alpha'));
...
// ==================>
  ...
  Future<bool> searchArea(BuildContext context, String searchText,
    ...
    if ((response.statusCode == 200 || response.statusCode == 201) &&
      ...
      for (int i = 0; i < data.serviceAreas.data.length; i++) {
        nameList.add(data.serviceAreas.data[i].serviceArea);
        idList.add(data.serviceAreas.data[i].id);
      }

      areaDropdownList = nameList;
      areaDropdownIndexList = idList;
      notifyListeners();
