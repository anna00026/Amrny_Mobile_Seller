...
class StateDropdownService with ChangeNotifier {
  ...
  Future<bool> fetchStates(BuildContext context,
    ...
    var response = await http.get(Uri.parse(
        '$baseApi/country/service-city/$selectedCountryId?page=$currentPage&order=alpha'));
...
//==============================>
  ...
  Future<bool> searchState(BuildContext context, String searchText,
    ...
    if ((response.statusCode == 200 || response.statusCode == 201) &&
      ...
      List idList = [];
      for (int i = 0; i < data.serviceCities.data.length; i++) {
        nameList.add(data.serviceCities.data[i].serviceCity);
        idList.add(data.serviceCities.data[i].id);
      }
      statesDropdownList = nameList;
      statesDropdownIndexList = idList;
